...
public class InitiateOutgoingWireTransferChallengeTests extends BaseSetup {
    ...
    public void AuthoriseOwt_Corporate_Success(final Currency currency, final OwtType type) throws Exception {
        ...
        final Pair<String, CreateOutgoingWireTransferModel> payment =
                OpenBankingPaymentInitiationHelper
                        .createAuthorisedOutgoingWireTransfer(sharedKey, headers, createOutgoingWireTransferModel, corporateAuthenticationToken, tppId);

        final ValidatableResponse response =
                OpenBankingSecureService.initiatePaymentChallenge(sharedKey, corporateAuthenticationToken, tppId, payment.getLeft())
                        .then()
                        .statusCode(SC_OK);
    ...
    public void AuthoriseOwt_Consumer_Success(final Currency currency, final OwtType type) throws Exception {
        ...
        final Pair<String, CreateOutgoingWireTransferModel> payment =
                OpenBankingPaymentInitiationHelper
                        .createAuthorisedOutgoingWireTransfer(sharedKey, headers, createOutgoingWireTransferModel, consumerAuthenticationToken, tppId);

        final ValidatableResponse response =
                OpenBankingSecureService.initiatePaymentChallenge(sharedKey, consumerAuthenticationToken, tppId, payment.getLeft())
                        .then()
                        .statusCode(SC_OK);
    ...
    private void assertSuccessfulResponse(final ValidatableResponse response,
        ...
        response.body("consent.id", notNullValue())
                .body("consent.state", equalTo("PENDING_CHALLENGE"))
