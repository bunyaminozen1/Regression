...
public class RejectOutgoingWireTransferTests extends BaseSetup {
    ...
    public void AuthoriseOwt_Corporate_Success(final Currency currency, final OwtType type) throws Exception {
        ...
        final Pair<String, CreateOutgoingWireTransferModel> payment =
                OpenBankingPaymentInitiationHelper.createOutgoingWireTransfer(sharedKey, headers, createOutgoingWireTransferModel);

        final ValidatableResponse response =
                OpenBankingSecureService.rejectPayment(sharedKey, corporateAuthenticationToken, tppId, payment.getLeft())
                        .then()
                        .statusCode(SC_OK);

        assertSuccessfulResponse(response, createOutgoingWireTransferModel, type, corporateId, OwnerType.CORPORATE);
    }
    ...
    public void AuthoriseOwt_Consumer_Success(final Currency currency, final OwtType type) throws Exception {
        ...
        final ValidatableResponse response =
                OpenBankingSecureService.rejectPayment(sharedKey, consumerAuthenticationToken, tppId, payment.getLeft())
                        .then()
                        .statusCode(SC_OK);

        assertSuccessfulResponse(response, createOutgoingWireTransferModel, type, consumerId, OwnerType.CONSUMER);
    }
    ...
    private void assertSuccessfulResponse(final ValidatableResponse response,
        ...
        response.body("consent.id", notNullValue())
                .body("consent.state", equalTo("REJECTED"))
