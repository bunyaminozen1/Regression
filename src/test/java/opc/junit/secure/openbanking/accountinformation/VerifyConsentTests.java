...
public class VerifyConsentTests extends BaseSetup {
    ...
    // TODO Check if these should work
    @Test
    public void AuthoriseConsent_Corporate_Success() {

        final String consentId =
                OpenBankingAccountInformationHelper.createConsent(sharedKey, headers);
        OpenBankingSecureServiceHelper.authoriseConsent(sharedKey, corporateAuthenticationToken, tppId, consentId);

        final ValidatableResponse response =
                OpenBankingSecureService.verifyConsent(sharedKey, corporateAuthenticationToken, tppId, consentId, VerificationModel.defaultVerification())
                        .then()
                        .statusCode(SC_OK);
    ...
    public void AuthoriseConsent_Consumer_Success() {
        ...
        final ValidatableResponse response =
                OpenBankingSecureService.verifyConsent(sharedKey, consumerAuthenticationToken, tppId, consentId, VerificationModel.defaultVerification())
                        .then()
                        .statusCode(SC_OK);
