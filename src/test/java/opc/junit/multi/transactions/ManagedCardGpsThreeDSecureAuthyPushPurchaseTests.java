...
public class ManagedCardGpsThreeDSecureAuthyPushPurchaseTests extends BaseGpsThreeDSecureSetup {

    /**
     * 3DS Authy Purchase
     * Tests do NOT focus on specific identities or transaction outcomes (auth and settlement), but
     * rather on integrated 3DS feature with authy push authentication factor, which happens before
    ...
    @Test
    public void ThreeDSecureCardAuthyPushPurchase_DebitAuthyAccept_Success() {
        ...
        SimulatePurchaseAndAcceptAuthyChallenge(managedCardId, new CurrencyAmount(currency, getRandomPurchaseAmount()), user.getLeft());

        //expect authorisation and settlement to occur since authy challenge was accepted
        ManagedCardsHelper.getManagedCardStatement(managedCardId, secretKey, token, 2)
                .then()
                .statusCode(SC_OK)
    ...
    @Test
    public void ThreeDSecureCardAuthyPushPurchase_PrepaidAuthyAccept_Success() {
    ...
    @Test
    public void ThreeDSecureCardAuthyPushPurchase_DebitAuthyReject_Success() {

        final Pair<String, String> user = createEnrolledUserAndVerifyAuthyPush();

        final String managedCardId = createDebitAuthyManagedCardAndSetSpendLimit(createManagedAccountAndDeposit(), user.getLeft());

        SimulatePurchaseAndRejectAuthyChallenge(managedCardId, new CurrencyAmount(currency, getRandomPurchaseAmount()));

        //expect authorisation and settlement not to occur since authy challenge was rejected
    ...
    @Test
    public void ThreeDSecureCardAuthyPushPurchase_PrepaidAuthyReject_Success() {
    ...
    @Test
    public void ThreeDSecureCardAuthyPushPurchase_InitiateBiometricNotEnabled_Success() {

        final Pair<String, String> user = createEnrolledUserAndVerifyAuthyPush();

        final String managedCardId = createDebitAuthyManagedCardAndSetSpendLimit(createManagedAccountAndDeposit(), user.getLeft());

        SimulateThreeDSecurePurchaseById(managedCardId, new CurrencyAmount(currency, getRandomPurchaseAmount()), false);

        //expect authorisation and settlement to occur since authy challenge was never issued
        ManagedCardsHelper.getManagedCardStatement(managedCardId, secretKey, token, 2)
                .then()
                .statusCode(SC_OK)
                .body("entry[0].transactionId.type", equalTo("SETTLEMENT"))
    ...
    @Test
    public void ThreeDSecureCardAuthyPushPurchase_InitiateBiometricNotProvided_Success() {
    ...
    public void ThreeDSecureCardAuthyPushPurchase_CardNotEnrolledForThreeDS_Success() {
        ...
        //Unable to initiate 3ds purchase since card is not 3ds authy enabled
