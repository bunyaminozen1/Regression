...
public class GetPermissionLoadTest extends Simulation {
    ...
    final ChainBuilder getPermission =
            exec(http("Get Permission")
                    .get(String.format("/multi_private/users/%s/permissions", "#{userId}"))
                    .queryParam("plugin", "PAYMENT_RUN")
                    .check(status().is(200)));
    final ScenarioBuilder getPermissionScenario = scenario("GetPermission")
            .feed(buyerFeeder)
            .exec(getPermission);

    {
        setUp(
                getPermissionScenario.injectClosed(constantConcurrentUsers(1).during(10),
